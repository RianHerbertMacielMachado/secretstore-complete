...
// Definição completa de cada método
const ALL_METHODS: {
  id: PaymentMethod
  label: string
  description: string
...
interface Props {
  /** Métodos configurados — detectados no servidor via process.env */
  enabledMethods: PaymentMethod[]
}

export default function CheckoutClient({ enabledMethods }: Props) {
  const { data: session } = useSession()
  const router = useRouter()
  const { items, getTotal, getSubtotal, getDiscount, coupon, clearCart } = useCartStore()

  // Filtra os métodos visíveis com base no que o servidor confirmou como configurado
  const visibleMethods = ALL_METHODS.filter((m) => enabledMethods.includes(m.id))

  const [paymentMethod, setPaymentMethod] = useState<PaymentMethod | null>(
    visibleMethods[0]?.id ?? null
  )
